...
exports.likeAd = (req, res) => {
  ...
  const likeDocument = db //find whether like it before
    .collection("likes")
...
//post ad with only image
exports.uploadOnlyAdImage = (req, res) => {
  console.log(req.params.adId);
  const BusBoy = require("busboy");
  const path = require("path");
  const os = require("os");
...
//post add+image at once
exports.postAdWithImg = (req, res) => {
  const BusBoy = require("busboy");
  const path = require("path");
  const os = require("os");
  const fs = require("fs");
